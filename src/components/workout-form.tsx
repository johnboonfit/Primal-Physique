...
import { createWorkout, getWorkoutDetail, updateWorkout, type ExerciseDraft } from '@/lib/workouts';
...
type ExerciseRow = {
  key: string;
  /** Set only when this row is an already-saved exercise being edited --
   * carried straight through to ExerciseDraft so updateWorkout() updates
   * it in place instead of treating it as new. */
  id?: string;
...
let nextSetKey = 0;
function makeSetRow(setNumber: number, setType: SetType = 'normal'): SetRow {
  nextSetKey += 1;
  return { key: `set-${nextSetKey}`, setNumber, setType };
}
...
/** Validates and converts one form row into what createWorkout()/
 * updateWorkout() actually save. Returns an error message (string)
 * instead of throwing, so the caller can point out which exercise the
 * problem is in. */
...
type WorkoutFormProps =
  | { workoutId?: undefined; weekId?: string }
  | { workoutId: string; weekId?: undefined };

/**
 * The coach's workout builder -- shared by both /workouts/new (a blank
 * form, optionally for one programme week's session) and /workouts/[id]
 * (the same form, preloaded from an existing workout for editing). Which
 * mode this is running in is decided entirely by whether `workoutId` was
 * passed in, not by two separate copies of ~600 lines of form code.
 */
export function WorkoutForm({ workoutId, weekId }: WorkoutFormProps) {
  ...
  const [effectiveWeekId, setEffectiveWeekId] = useState<string | null>(weekId ?? null);
  ...
  // When editing, load the existing workout and turn it into the exact
  // same row shape a freshly-built form uses -- everything below this
  // (search, add/remove exercise, tag a set...) works identically either
  // way once the rows are in state.
  useEffect(() => {
    if (!workoutId) return;
    let cancelled = false;
    getWorkoutDetail(workoutId)
      .then((detail) => {
        ...
        setEffectiveWeekId(detail.programmeWeekId);
        setExercises(
          detail.exercises.length > 0
            ? detail.exercises.map((exercise) => ({
                key: `exercise-existing-${exercise.id}`,
                id: exercise.id,
                ...
                searchQuery: '',
                baselineWeight: exercise.baselineWeight !== null ? String(exercise.baselineWeight) : '',
                baselineReps: exercise.baselineReps !== null ? String(exercise.baselineReps) : '',
                taggedSets: exercise.sets.map((set) => makeSetRow(set.setNumber, set.setType)),
              }))
            : [makeExerciseRow()]
        );
      })
      .catch((err) => {
        if (!cancelled) setLoadError(err instanceof Error ? err.message : 'Failed to load this workout.');
      })
      .finally(() => {
  ...
  // Only fetched when there's a week to show context for -- purely
  // informational, doesn't affect saving.
  useEffect(() => {
    if (!effectiveWeekId) return;
    let cancelled = false;
    getProgrammeWeekContext(effectiveWeekId)
      .then((context) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (workoutId) {
        await updateWorkout(workoutId, name, cleaned);
      } else {
        await createWorkout(session.user.id, name, cleaned, weekId);
      }
      if (effectiveWeekId) {
        router.replace(`/programmes/week/${effectiveWeekId}`);
      } else {
    ...
    return (
      <ThemedView style={styles.container}>
        <SafeAreaView style={styles.safeArea}>
          <ActivityIndicator style={styles.libraryLoader} />
        </SafeAreaView>
      </ThemedView>
    );
  }
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          <ThemedText type="title" style={styles.title}>
            {isEditing ? 'Edit workout' : weekId ? 'New session' : 'New workout'}
          </ThemedText>
          {effectiveWeekId && weekContext && (
