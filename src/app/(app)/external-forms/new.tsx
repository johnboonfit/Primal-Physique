...
import { createExternalForm } from '@/lib/external-forms';
...
/**
 * The External Builder — same question-type system as the Check-in
 * Builder (forms/new.tsx), just saving into external_forms/
 * external_form_questions instead: one-off and shareable rather than
 * recurring and tied to a client. This screen is intentionally a
 * near-copy of forms/new.tsx rather than a shared abstraction — the two
 * builders save to genuinely different tables with different downstream
 * behavior (recurring assignment vs. a public link), and forcing one
 * shared component to branch on "which kind of form" would be the
 * premature abstraction, not the duplication.
 */
export default function NewExternalFormScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const formId = await createExternalForm(session.user.id, trimmedName, drafts);
      router.replace(`/external-forms/${formId}`);
    } catch (err) {
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <ThemedText type="small" themeColor="textSecondary" style={styles.hint}>
            Generates a public link — anyone with it can view and submit, no account needed.
          </ThemedText>

          <TextInput
            value={name}
            onChangeText={setName}
            placeholder="Form name (e.g. PAR-Q Health Screening)"
  ...
  return (
    <ThemedView type="backgroundElement" style={styles.questionCard}>
      ...
      <TextInput
        ...
        placeholder="Question text (e.g. Do you feel pain in your chest during activity?)"
        placeholderTextColor={theme.textSecondary}
        style={[styles.input, { color: theme.text, borderColor: theme.backgroundSelected }]}
      />
