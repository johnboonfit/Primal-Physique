...
import { getMealPlanTemplateDetail, updateMealPlanTemplateDetails } from '@/lib/meal-plans';
...
export default function EditMealPlanScreen() {
  ...
  useEffect(() => {
    ...
    getMealPlanTemplateDetail(id)
      .then((template) => {
        ...
        setProteinPercent(String(template.targetProteinPercent));
        setCarbPercent(String(template.targetCarbPercent));
        setFatPercent(String(template.targetFatPercent));
      })
      .catch((err) => setLoadError(err instanceof Error ? err.message : 'Failed to load this template.'))
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateMealPlanTemplateDetails(id, {
        name: trimmedName,
        goalType,
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { marginTop: Spacing.two, marginBottom: Spacing.half },
  ...
  rowItem: { flex: 1, gap: Spacing.half },
  percentWarning: { color: Accent },
