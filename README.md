...
src/
  app/
    ...
    (app)/
      ...
      challenges/
        index.tsx        # shared by both audiences: coach sees every challenge they've created with a participant count and a "+ New" link; a client sees active/upcoming challenges they're eligible for with a genuine Join/Leave button (join/leave only for now — no scoring or leaderboard yet, see challenges.ts)
