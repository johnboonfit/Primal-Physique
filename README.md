  ...
  app/
    ...
    (app)/
      ...
      client/
        ...
        training.tsx      # Training tab — a top <PillRow> (View Calendar / Log Activity / Form Check, same shared pill component Progress's own switcher uses) directly under the title, then the Volume Analyser card (this week's per-muscle-group set counts, see muscle-group-analysis.ts), a "Recent Activities" list (per-entry Delete, see activity-logs.ts — logging itself happens via the top pill's <LogActivityModal>, not a card here), Your Programme card (week counter, day row, next workout), and full assignment history; Form Check itself is nav-only now (no inline preview/locked card) — /client/form-check already shows the right enabled/locked state on its own
