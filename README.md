...
**What was verified here, and what needs your own device:** this sandbox's network proxy blocks both `world.openfoodfacts.org` and `api.nal.usda.gov` outright, so the actual live API responses — whether Open Food Facts' UK filters really return what's expected in practice — couldn't be exercised end-to-end here. What *was* verified, against realistic canned API responses run through the real production code: the merge order (UK results ranked first), de-duplication (a product returned by both OFF passes appears once), the quality gate (a real UK supermarket product that doesn't match the search term is correctly excluded, and an unrelated search like "chicken breast" shows no UK results at all, same as before this change), and all three portion-unit modes computing the right total grams and macros (a structured USDA portion, a multi-count of one, and a manual custom item).
...
src/
  ...
  lib/
    ...
    open-food-facts.ts       # getProductByBarcode() — live barcode lookup, used by the scanner; searchUKFoods() — the UK-supermarket-targeted v2 API pass blended into typed search (see food-search.ts); legacy searchFoods() (cgi/search.pl) built but unused; portions on every result, from a real serving_quantity when Open Food Facts has one
