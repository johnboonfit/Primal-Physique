...
**How you verify this yourself:** run `supabase/activity-logs.sql` in the Supabase SQL Editor (after `custom-exercises.sql`, per its own header comment). As a client, go to Training, tap **+ Log Activity**, pick a type (or **+ Custom** and type your own), fill in the duration, and save — confirm it appears in the list below with a **Delete** action. Check your Momentum ring on Home — it should tick up slightly once you've logged an activity this week. As the coach, check the Home dashboard's Recent Activity preview and the full Activity feed screen — the logged activity should appear on both, with the client's name, what they did, and how long.
...
src/
  app/
    ...
    (app)/
      ...
      client/
        ...
        training.tsx      # Training tab — Volume Analyser card (this week's per-muscle-group set counts, see muscle-group-analysis.ts) directly under the hero stat, then a Form Check card (real record/upload entry point into form-check.tsx, or the locked upsell card if the coach has toggled it off) + Log Activity section (a "+ Log Activity" pill opening <LogActivityModal>, a recent-activities list with per-entry Delete, see activity-logs.ts) + Your Programme card (week counter, day row, next workout) + full assignment history + "View Calendar →" pill (bordered, matching Log Activity's own pill style)
  ...
  components/
    ...
    log-activity-modal.tsx           # <LogActivityModal visible clientId onClose onSaved> — the client's Log Activity flow from Training: activity-type chip grid (+ a "+ Custom" dashed chip revealing a free-text name), duration (required), distance + km/mi toggle/calories/notes (optional); same modal/chip shape nutrition.tsx's food-add modal already established
