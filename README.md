...
**The heat-map is a muscular body silhouette modeled on a standard front/back anatomy reference chart** — arms held slightly out from the torso (so the lat/armpit area actually reads instead of an arm flattened against the body), distinct trapezius/deltoid/pectoral/lat regions, a segmented ab grid, and a back view with a real trapezius-to-lat V-taper, each region mirrored left-right off a single authored path rather than hand-duplicated, with a thin dark outline separating adjacent regions the way a muscle-map diagram would. A soft teal ambient glow sits behind the whole diagram (`Glow.teal`, the same treatment `<StatRing>` already uses), so it reads as part of this app's visual language rather than a bolted-on generic chart. Tap anywhere on it to flip between front and back. Chest and core only appear on the front (not visible from behind); back only appears on the back; shoulders/arms/legs/calves appear identically on both since they're visible either way and always carry the same count.
...
src/
  ...
  components/
    ...
    muscle-heatmap.tsx               # <MuscleHeatmap counts> — tap-to-flip SVG muscular body silhouette modeled on a front/back anatomy chart (path-based regions, mirrored left/right off one authored side, arms held out from the torso), soft teal ambient glow (Glow.teal) behind it; color-coded per region by colorForCount() (zero = neutral, no tier color); exports TIER_COLORS (bright teal -> deep teal -> oxblood, an on-brand intensity gradient) and colorForCount() so the analyser card's list/badge use the exact same colors
