...
## Log Activity (Training tab)

**What/why**: there was nowhere for a client to record a run, swim, ride, or anything else done outside their prescribed programme — cardio/conditioning work simply went untracked. The Training tab now has a **Log Activity** section: a pill-style "+ Log Activity" button (matching the pill pattern Nutrition's own quick-add already established) opens a modal to log one of eight fixed activity types (Run/Swim/Bike/Walk/Row/Pilates/Stairmaster/Activity) or a free-typed custom one, with duration (required), distance + km/mi toggle, calories, and notes (all optional beyond duration). Logged activities show in a small recent list right there on Training, each with a **Delete** action, so logging one is never a black box. **Also**: Training's "View Calendar" link is now a bordered pill too, for the same reason — visual consistency with the new Log Activity pill sitting right next to it.

**Feeds the coach's Activity feed**: exactly the same real-time cross-client feed food logs, habits, and completed workouts already flow into (see `client-activity-feed.sql` from an earlier chunk) — both the small dashboard preview (`getRecentClientActivity()`) and the full feed screen (`getClientActivityFeed()`) now include a `kind: 'activity'` event, carrying the activity's label, duration, distance, and calories. New RLS policy gives the coach read access to every client's `activity_logs`, the same shape the habit-log coach-read policy already established.

**Factors into Momentum Score — two ways, not one**: a logged activity counts as "did something today" exactly like a completed workout, a logged meal, or a logged habit — it's folded into the same `activeDays` set the other three already build, so it can lift `activeDaysRate` precisely like they do. On top of that, logging at least one activity in the current week adds a flat **+0.5** bonus to the final score (capped at 10) — not every client's programme includes cardio, so this is a direct, guaranteed reward for optional conditioning work the other four rate components wouldn't otherwise recognize on their own (folding it into `activeDaysRate` alone wouldn't move the needle on a day the client also logged food or a habit). `MomentumBreakdown` now exposes `activityLoggedThisWeek` so this is visible/debuggable, even though today's UI only ever shows the final rolled-up score.

**New**: `supabase/activity-logs.sql` (the `activity_logs` table + RLS: a client can insert/select/delete only their own rows, a coach can select every client's); `src/lib/activity-logs.ts` (`ACTIVITY_TYPES`, `addActivityLog()` / `listMyActivityLogs()` / `deleteActivityLog()` / `activityLabel()`); `src/components/log-activity-modal.tsx` (the modal itself, chip-grid + km/mi toggle, same modal/chip shape Nutrition's food-add modal already established). **Changed**: `client/training.tsx` (Log Activity section + pill, recent list, the View Calendar pill), `momentum.ts` (activity days folded into `activeDaysRate`, the +0.5 bonus), `coach-dashboard.ts` (`ActivityEvent`/`ClientActivityEvent` gain an `'activity'` variant, both feed functions and the realtime subscription updated), `home.tsx`/`activity.tsx` (render the new event kind).

**Verified here** two different ways. First, for real: applied `activity-logs.sql` against a real Postgres instance — confirmed a client can log their own activity, a client can't log one claiming to be someone else, the coach can't log an activity at all (client-only insert), a bad activity type or a zero/negative duration is rejected by a CHECK constraint, a different client can't see or delete someone else's entry, the coach CAN see every client's entries but can't delete any of them (read-only), and a client can delete their own. Second, the app side: a generic in-memory fake of `supabase.from()`, rendered through the real Training tab — logged a Run (40min/8km/350cal) and a custom "Kickboxing Class", confirmed both show correctly in the recent list, confirmed Save stays disabled until an activity type AND a duration are both set, confirmed Delete removes an entry from the underlying data. Directly exercised the real `getMomentumScore()` before and after removing the only activity logged that week and confirmed the score dropped by exactly the +0.5 bonus plus that day's `activeDaysRate` share — not a made-up number, the exact arithmetic the function itself uses. Directly exercised the real `getRecentClientActivity()` and `getClientActivityFeed()` and confirmed both return a correctly-shaped `activity` event; separately rendered the real coach Home dashboard and the real Activity feed screen and confirmed the logged activity appears on both with the right client name, label, duration, distance, and calories.

**How you verify this yourself:** run `supabase/activity-logs.sql` in the Supabase SQL Editor (after `custom-exercises.sql`, per its own header comment). As a client, go to Training, tap the **Log Activity** pill, pick a type (or **+ Custom** and type your own), fill in the duration, and save — confirm it appears in the "Recent Activities" list below with a **Delete** action. Check your Momentum ring on Home — it should tick up slightly once you've logged an activity this week. As the coach, check the Home dashboard's Recent Activity preview and the full Activity feed screen — the logged activity should appear on both, with the client's name, what they did, and how long. (Note: a later chunk moved the Log Activity trigger from its own standalone button into the top pill row alongside View Calendar/Form Check — see "Consistent pill row" below.)

## Consistent pill row (Training, Nutrition, Progress)

**What/why**: Progress's Metrics/Measure/Exercise/Photos switcher already had a clean pill look (rounded-full, `Colors.backgroundElement` resting / `Accent` selected) — but every other screen with similar top-level actions had grown its own one-off styling instead: Training's "View Calendar" was a bordered outline pill, "Log Activity" was a solid-filled oxblood-glow button, and "Form Check" was a full descriptive card; Nutrition's "Saved Meals" was a plain text link in the header. Three different looks for the same kind of thing (a row of top-level actions right under the screen's title) reads as inconsistent, and was only going to keep drifting further apart with each new chunk. Extracted the ONE pill look into a shared `PillRow` component, and Progress itself was switched to use it too — so all four screens now render literally the same styles, not just visually-similar copies that could quietly diverge again later.

**Training**: "View Calendar", "Log Activity", and "Form Check" are now three pills in one row directly under the title, replacing the old outline pill, filled button, and descriptive card respectively. "Form Check" is a plain navigation pill now (no more inline preview/locked card on Training itself) — `/client/form-check` already shows the correct enabled/locked state on its own, so duplicating that check on Training was redundant. The old standalone "+ Log Activity" button and its "Log Activity" section label are gone too — the top pill is now the only way to open the modal; the "Recent Activities" list underneath (with per-entry Delete) is unchanged.

**Nutrition**: "Saved Meals" is now a pill in its own row under the title, matching Training/Progress, instead of a plain text link sharing the header row with the quick-add search icon (which stays exactly where it was).

**New**: `src/components/pill-row.tsx` (`<PillRow items activeKey?>` — `activeKey` is what lets Progress's switcher render one pill in the selected/Accent-filled state; a row of plain navigation pills like Training's or Nutrition's just leaves it unset, so every pill renders in the same resting look). **Changed**: `progress.tsx` (refactored onto `PillRow`, its own copy of the pill styles removed entirely — it's now downstream of the shared component, not a separate parallel implementation), `client/training.tsx`, `client/nutrition.tsx`.

**Verified here** with a Playwright pass against a generic in-memory fake of `supabase.from()`, rendered through the real screens — confirmed Training's three pills and Nutrition's one pill all read back the exact same computed `borderRadius`/`backgroundColor` as one of Progress's own pills (not just "looks about right" — the literal computed CSS values matched), confirmed the old Form Check preview card and the old standalone "+ Log Activity" button text are both gone from Training, and confirmed all three of Training's pills and Nutrition's pill still do their job — View Calendar and Form Check navigate to the right screen, Log Activity still opens the same modal, Saved Meals still navigates to the Saved Meals screen.

**How you verify this yourself:** open Progress, Training, and Nutrition side by side (or one after another) — the pills should look identical in shape and color across all three. On Training, confirm View Calendar/Log Activity/Form Check all still work exactly as before, just relocated into one pill row under the title. On Nutrition, confirm Saved Meals still opens the Saved Meals screen from its new pill.

## Pills always red/white, Training/Home rework

**Pills, always red/white**: the pill row above rendered every navigation pill (Training's three, Nutrition's one) in the plain resting look — dark background, grey text — since none of them ever had a "selected" state to contrast against. Turns out that read as inconsistent with what a pill on Progress actually looks like day to day (the selected sub-tab is red/white). `PillRow` now treats a row with no `activeKey` at all (every plain navigation row) as "every pill is the selected/red-and-white one" by default; Progress's own switcher is unaffected, since it always passes a real `activeKey` and still shows exactly one pill red at a time, the rest in the resting look.

**Training reworked**: the "Workouts Assigned" hero box is gone entirely — that count now lives on Home instead (see below), so Training doesn't show it twice. With it gone, the Volume Analyser card ("This Week") is now the literal first thing under the pill row, nothing sitting above it.

**Home gains two new rings**: "Workouts" (the exact same assignment count Training's hero box used to show — reuses data Home was already fetching for Up Next, no new query) and "Weight" (the client's most recent weigh-in, pulled from the same `weight_logs` table Progress → Metrics reads and writes). Both are pressable, same as the existing Calories ring — Workouts opens Training, Weight opens Progress. Weight shows a muted "Log one in Progress" placeholder for a client who's never logged a weigh-in yet, same honest-placeholder convention Steps/Readiness/Momentum already use when there's nothing real to show.

**Home reordered**: "Up Next" now renders after the streak/XP row instead of before the ring grid — new order is Ring Grid → streak/XP → Up Next → Today's Habits (was: Up Next → Ring Grid → streak/XP → Today's Habits). Pure reorder, no behavior change.

**New**: `getLatestWeightLog()` in `weight-logs.ts` (a single-row read of the most recent weigh-in, rather than fetching the client's entire history just to show one number). **Changed**: `pill-row.tsx` (the default/no-`activeKey` case), `client/training.tsx` (hero box removed), `client/index.tsx` (two new rings, Up Next moved).

**Verified here** with a Playwright pass against a generic in-memory fake of `supabase.from()` — confirmed Home's new Workouts ring shows the right assignment count and opens Training, confirmed the Weight ring shows the most recent of two logged weigh-ins (not the older one) with its log date as the subtitle and opens Progress, confirmed the new section order (streak/XP, then Up Next, then Today's Habits) by checking each heading's position in the rendered page, confirmed Training no longer shows "Workouts Assigned" anywhere and that the Volume Analyser section is now the first content under the pill row, and re-confirmed Training's pills are still red/white after these changes (nothing in this pass regressed the earlier pill-consistency work).

**How you verify this yourself:** open Home — you should see two new rings, "Workouts" and "Weight", in a third row under Steps/Calories; tap each to confirm they open Training and Progress respectively. Confirm "Up Next" now appears below your streak/Level card and above "Today's Habits". On Training, confirm there's no "Workouts Assigned" box anymore and the Volume Analyser card is the first thing you see under the pill row.

## Upgrade Plan (Stripe)

**What/why**: a client had no way to upgrade their own membership tier in-app before now — a coach had to set it by hand (see `community-leaderboards.sql`'s own comment: *"There's no live Stripe -> Supabase sync yet"*). A prominent **⭐ Upgrade Plan** button now sits directly under Settings' hero card (email + current plan), for clients only — tapping it opens a dropdown of the three real tiers (**Request New Plan (Base Members)**, **Accelerator Plan**, **Precision Plan**), and picking one hands off to a real, live Stripe Checkout using the coach's actual Price IDs.

**How the money side actually works, end to end**: tapping a plan first inserts the client's own `pending` row into a new `plan_upgrade_requests` table (client-side, under normal RLS — a client can only ever start a request for themselves). That row's id — never the tier itself — is handed to a new Supabase Edge Function, `create-checkout-session`, which looks the request back up itself (so a tampered client request can't ask for a cheaper plan's price), maps its tier to the matching Stripe Price, and asks Stripe for a real Checkout Session. The app opens that session in an in-app browser (`expo-web-browser`, already an unused dependency in this project — no new package needed) via `WebBrowser.openAuthSessionAsync`, which closes itself the moment Stripe redirects back to the app's own `primalphysique://` scheme. **The client's tier does not change at this point** — only once Stripe actually confirms payment does a second Edge Function, `stripe-webhook`, hear about it (`checkout.session.completed`) and upgrade `client_tiers` for real, using the service-role key the same way `delete-client`'s function already does — a client was never given permission to write their own tier directly (that's deliberate, unchanged from `community-leaderboards.sql`), so this webhook is the one new privileged write path.

**Honest limits of this first version** — worth knowing before relying on it: every upgrade always starts a **brand-new** Stripe subscription; there's no `stripe_customer_id` tracked per client yet (there never has been, before this chunk), so a client who upgrades a second time to a different tier ends up with two active subscriptions rather than one modified in place — reconciling that today is a manual step in the Stripe dashboard, not something this code resolves on its own. Cancellations, failed renewals, and refunds also aren't handled by `stripe-webhook` yet — it only ever reacts to a successful checkout. Both are real, scoped gaps, not silently swept under the rug — a future chunk adding Stripe's Customer Portal (which needs that `stripe_customer_id` tracked first) is the natural next step if this becomes a problem in practice.

**New**: `supabase/plan-upgrades.sql` (the `plan_upgrade_requests` table + RLS), `supabase/functions/create-checkout-session/index.ts` and `supabase/functions/stripe-webhook/index.ts` (two new Edge Functions — deployment steps below), `src/lib/plan-upgrades.ts` (`PLAN_UPGRADE_OPTIONS`, `startPlanUpgrade()`). **Changed**: `settings.tsx` (the button + a real modal dropdown, client-only).

**Verified here** two different ways. First, for real: applied `plan-upgrades.sql` against a real Postgres instance — confirmed a client can start their own pending request, can't start one for someone else or pre-mark one completed (both rejected outright by the insert policy, not just hidden by the UI), the coach can't start one at all (client-only action), a different client can't see someone else's request, the coach CAN see every request, and — critically — the client herself can't mark her own request completed (0 rows affected; there's no update policy for anyone, on purpose); then simulated the webhook's own privileged write (the same bypass a service-role key gets) and confirmed the upsert-into-`client_tiers` + mark-completed sequence works and reads back correctly through the exact query `getMyTier()` already uses. Second, the app side: a generic in-memory fake of `supabase.from()`/`supabase.functions.invoke()`/`WebBrowser.openAuthSessionAsync`, rendered through the real Settings screen — confirmed the button only shows for a client (never a coach), the dropdown shows the exact three labels, selecting one inserts the right pending row, calls the Edge Function with that row's id (not a bare tier — proving the anti-tampering design actually works, not just reads that way in the code), opens the in-app browser with the URL the function returned and the right return scheme, shows a confirmation notice after a simulated successful checkout, and — separately — that a real Edge Function error (e.g. a missing secret) surfaces its actual message in the UI instead of a generic failure, with the dropdown staying open so the client can retry.

**What I could NOT verify, and why**: the two Edge Functions' own Stripe-facing logic (creating a real Checkout Session, verifying a real webhook signature) never actually ran against Stripe in this pass — there's no Deno runtime in this environment to execute them (installing one was blocked by network policy), and doing so for real would need your live Stripe secret key, which this session was never given and should never be pasted into a chat. Both files mirror `delete-client`'s already-deployed, working structure exactly (same two-client caller/admin pattern, same CORS/response helpers) and use Stripe's documented Node SDK methods, but you should smoke-test them yourself in Stripe **test mode** (card `4242 4242 4242 4242`, any future expiry, any CVC) before relying on this for a real charge.

**How you deploy this yourself** (nothing here happens automatically — same as every migration in this project, you paste and run these):
1. Run `supabase/plan-upgrades.sql` in the Supabase SQL Editor (after `activity-logs.sql`, per its own header comment).
2. In the Supabase Dashboard: **Edge Functions → Create a new function** → name it `create-checkout-session` → paste in `supabase/functions/create-checkout-session/index.ts`'s contents → Deploy.
3. Same again for `stripe-webhook` (paste `supabase/functions/stripe-webhook/index.ts`) — but this one **must** have "Enforce JWT Verification" turned OFF in its settings (Stripe calls it directly, with no Supabase session at all; the CLI equivalent is `supabase functions deploy stripe-webhook --no-verify-jwt`).
4. Under **each** function's own Edge Functions → Secrets: set `STRIPE_SECRET_KEY` (same value, both functions) and, on `stripe-webhook` only, `STRIPE_WEBHOOK_SECRET`.
...
## Coach-side notification bubbles: new clients, unread messages, completed check-ins

**What/why**: the client side has had live unread badges (Chat tab, Community) for a while now — the coach side had nothing, so a new client signing up, a new message coming in, or a client finishing a check-in were all things the coach could only find by going and looking. Three new numbered red badges now sit on Home's Manage grid: **Clients** (new signups since the roster was last opened), **Messages** (unread messages across every conversation, not just one), and a brand-new **Check-ins** card (completed submissions since that list was last opened) — each shows nothing at all when its count is zero, same "no badge, not a badge showing 0" convention `tabBarBadge` already uses on the client side.

**Same pattern throughout, nothing new invented**: a `*_last_viewed_at` timestamp on the coach's own `profiles` row (`roster_last_viewed_at`, `checkins_last_viewed_at` — both new; the Messages badge needed no new column at all, since `conversation_reads.last_read_at` already means exactly this per-conversation), compared against the relevant table's own timestamp (other profiles' `created_at`, `form_check_ins.completed_at`), updated the instant the destination screen actually opens, and kept live afterward by a realtime subscription that just triggers a refetch — the exact shape `unread-badges.sql`/`community.ts` established for the client's own Chat/Community badges. The Messages badge is the same per-conversation unread-count math `getUnreadMessageCount()` already does for the client's Chat tab, just summed across every conversation the coach has instead of one at a time — each conversation keeps its own last-read threshold, so a client the coach hasn't messaged in weeks doesn't count as unread just because a different thread was read more recently.

**Check-ins is a genuinely new screen**, not just a new badge — there was no coach-facing list of completed check-in submissions before this (only `checkins/[id].tsx`, a shared detail view reached from elsewhere). `checkins/index.tsx` lists every completed submission across every client, most recent first, each tapping through to that same existing detail screen; opening this new list is what marks check-ins viewed, same "seeing it counts as reading it" rule every other badge in this app follows.

**New**: `supabase/coach-notifications.sql` (the two new `profiles` columns + their column-level grants, `profiles`/`form_check_ins` added to the realtime publication); `src/app/(app)/checkins/index.tsx`. **Changed**: `clients.ts` (`getRosterLastViewedAt()`, `markRosterViewed()`, `getNewClientCount()`, `subscribeToNewClients()`), `chat.ts` (`getCoachUnreadMessageCount()`, `subscribeToCoachInbox()`), `form-check-ins.ts` (`listCompletedCheckIns()`, `getCheckinsLastViewedAt()`, `markCheckinsViewed()`, `getNewCompletedCheckInCount()`, `subscribeToCoachCheckIns()`), `clients/index.tsx` (marks the roster viewed on focus), `home.tsx` (`NavCard` gains a `badge` prop — a small red circle, top-right corner, hidden entirely at zero — wired onto Clients/Messages, plus a new Check-ins card).

**Verified two ways.** For real: applied `coach-notifications.sql` against a real Postgres instance (with the actual `is_coach()`-based "coach sees every client" RLS policy in place, not a simplified stand-in) — confirmed both columns exist as `timestamptz not null default now()`, both tables land on `supabase_realtime`, every existing row gets the default (nothing retroactively shows as "new" the moment this ships), a client can update their own `roster_last_viewed_at` and the coach can update both of their own new columns, a client is blocked from touching the coach's timestamps (RLS) and blocked from touching any *other* column on their own row even though they own it (the grant is genuinely column-restricted, not a free pass on the whole row), the count-shaped queries return the right numbers, and re-pasting the whole file a second time is safe. Second, the app itself: a hand-rolled fake `supabase` client (patched `.from()`/`.channel()`/`.auth`) rendered through the real Home screen — confirmed all three badges show the exact counts the mock data implies (3/4/7), styled as small red circles with white numerals in each card's top-right corner, and confirmed separately that a zero count renders no badge at all.

**How you deploy this**: run `supabase/coach-notifications.sql` in the Supabase SQL Editor, after `plan-upgrades.sql`.
...
**What/why**: a client can now upload a real profile picture, replacing the initials-in-a-circle placeholder everywhere it showed — Settings' Hero card, and (new) a small circle next to the gear icon on the client's own Home tab, so it's visible without having to open Settings at all.

**Tapping the Hero card avatar** (client only — a coach still just sees their initials, unchanged, since only the client side has an upload button) opens a small menu: Take Photo, Choose from Library, and — once a photo exists — Remove Photo, reverting back to initials. Uses the exact same `expo-image-picker` flow (square crop, `base64: true`, decoded to an ArrayBuffer for upload) every other image upload in this app already uses, since React Native's normal Blob/File/FormData path doesn't work reliably against Supabase Storage.

**Storage follows the same private-bucket shape as progress photos/recipe covers** (`progress-photos.sql`/`recipes.sql`): a private `avatars` bucket, one file per user under their own `<user_id>/` folder, RLS on `storage.objects` keyed on that folder prefix matching `auth.uid()`. Every upload gets a fresh, timestamped filename rather than overwriting one fixed path in place — the previous file, if any, is only deleted once the new one is safely recorded on the profile, so a failure partway through a re-upload never leaves the profile pointing at nothing. Since files are never overwritten in place, this needed no `UPDATE` policy on `storage.objects` at all, just select/insert/delete. The bucket is private, so there's no permanent public URL to store — `profiles.avatar_storage_path` (new, nullable column) just records which file is current, and a fresh signed URL is generated on demand (1 hour, same window progress-photos.ts already uses) wherever it's actually displayed.

**New**: `supabase/profile-avatar.sql` (the `avatars` bucket + its storage RLS, the `avatar_storage_path` column + its column-level grant); `src/lib/profile-avatar.ts` (`initials()` — moved here from being duplicated locally in `settings.tsx` — plus `getAvatarUrl()`, `uploadAvatar()`, `removeAvatar()`). **Changed**: `settings.tsx` (Hero card avatar is tappable for a client, with the picker/remove menu), `client/index.tsx` (the new small avatar next to the gear icon, refetched on every focus so a photo just uploaded in Settings shows up the moment this tab is back in view — not only after a fresh app launch).

**Verified two ways.** For real: built a structurally accurate stand-in for Supabase's actual `storage.buckets`/`storage.objects`/`storage.foldername()` schema (the real Storage RLS layer, not a simplified guess) against a real Postgres instance, applied `profile-avatar.sql`, and confirmed the bucket is private, a user can insert/select/delete only under their own folder and is rejected outright touching anyone else's, `avatar_storage_path` is nullable (an existing row is never retroactively broken), a user can update only their own row's column, and re-pasting the whole migration a second time is safe. Second, the app itself: rendered the real Settings and client Home screens through a hand-rolled fake Supabase client, toggling between "no avatar" and "has an avatar" — confirmed the initials circle (with a small edit-pencil badge) shows when there's no photo, a real `<Image>` replaces it the instant one exists, "Remove Photo" only ever appears in the menu when there's actually something to remove, and the same small circle correctly renders next to Home's gear icon in both states. The one thing that could not be verified here: actually driving the OS's own camera/photo-library picker UI, which is native, device-level chrome outside what this environment (or Playwright) can exercise — the code path feeding it (permission request, launch, upload, cleanup) mirrors the already-shipped-and-working Progress Photos feature's picker call exactly.

**How you deploy this**: run `supabase/profile-avatar.sql` in the Supabase SQL Editor, after `coach-notifications.sql`.
...
src/
  app/
    ...
    (app)/
      home.tsx          # coach's home screen — a real dashboard (stat tiles, Needs Attention, a merged real Recent Activity preview with a "View all →" into activity.tsx) plus Manage/Coaching Hub nav grids covering every coach screen (including "Challenges", "Resource Library", and "Form Check" cards); Clients/Messages/Check-ins each carry a live numbered badge (new signups/unread messages/new completions since last opened — see coach-notifications.sql), hidden entirely at zero; a gear icon opens /settings (Sign out lives there now, not here); redirects clients to /client
      settings.tsx      # shared coach/client Settings screen — Hero card (real uploaded photo if a client has set one, initials otherwise; tapping it is client-only — Take Photo/Choose from Library/Remove Photo, see profile-avatar.ts — email, real tier or "Coach") + a client-only "⭐ Upgrade Plan" button right beneath it (opens a real modal dropdown of the three tiers, hands off to a real Stripe Checkout via plan-upgrades.ts — see the dated section for how) + Profile settings card (name/email/phone, one Save; email changes go through Supabase Auth's real confirm-by-link flow, never an instant profiles.email overwrite) + Notification toggles card (4 switches, save-on-flip, preference storage only — no delivery built yet) + Wearable card (real getWearableConnections() read — shows "Not connected"/"Never" until a native HealthKit/Health Connect integration exists to actually write a connection, Force Sync stays disabled) + a Sign out card + an App Version card (Constants.expoConfig?.version, straight off app.json — the true last thing on the screen now)
      activity.tsx      # coach-only "Client Activity" — the full, real-time, cross-client feed (meals/habits/completed workouts/logged activities, each with that client's live Momentum + Compliance Score); see getClientActivityFeed()/subscribeToClientActivity() in coach-dashboard.ts
      messages/
      ...
      clients/
        _layout.tsx      # coach-only guard for everything below
        index.tsx        # list of every client account, each row showing a color-coded Compliance Score % badge, a Tier: Base/Accelerator/Precision picker (coach-set, gates that client's Leaderboards access), a "Paused" badge when relevant, and a Pause/Reactivate action; opening this list marks the roster viewed, clearing Home's Clients nav card badge
      ...
      checkins/
        index.tsx         # coach-only — every completed check-in submission across every client, most recent first, each tapping through to [id].tsx below; opening this list marks check-ins viewed, clearing Home's Check-ins nav card badge
      ...
      client/
        ...
        index.tsx        # Home tab — greeting + a small profile-picture circle (real photo or initials, same as Settings' Hero card — see profile-avatar.ts) next to the gear icon opening /settings (Sign out lives there now, not here) + daily logging nudge + a 3x2 ring grid (Readiness [real, see training-readiness.ts] / Momentum [greyed if the coach has toggled it off] / Steps [real getDailyMetricsForDate() read, muted placeholder until a wearable actually syncs] / Calories Today / Workouts [the same assignment count Training's old hero box showed] / Weight [most recent weigh-in from weight_logs, opens Progress; muted "Log one in Progress" placeholder if there's none yet]) + streak/Level/XP cards + Up Next (merges pending workouts + due check-ins — now BELOW streak/XP, not above the ring grid) + weekly TDEE recalculation check + Today's Habits checklist + a "Challenges" card into /challenges + a "Resource Library" card into /resources (where Community's old card used to sit — Community is its own tab now, see _layout.tsx)
        training.tsx      # Training tab — a top <PillRow> (View Calendar / Log Activity / Form Check, same shared pill component Progress's own switcher uses) directly under the title, then the Volume Analyser card as the first content (this week's per-muscle-group set counts, see muscle-group-analysis.ts — the "Workouts Assigned" hero box that used to sit above it is gone; that count now lives on Home's own ring grid instead), a "Recent Activities" list (per-entry Delete, see activity-logs.ts — logging itself happens via the top pill's <LogActivityModal>, not a card here), Your Programme card (week counter, day row, next workout), and full assignment history; Form Check itself is nav-only now (no inline preview/locked card) — /client/form-check already shows the right enabled/locked state on its own
        nutrition.tsx      # Nutrition tab — header (title + quick-add search icon) → a <PillRow> with just "Saved Meals" → ‹›date navigator → one glowing summary card (calorie ring + Protein/Carbs/Fat target bars, see macros.ts), 4 meal sections each with "Save as meal" (bundles its currently-logged entries into a new saved_meals template, see saved-meals.ts) alongside "+ Add", blended USDA + UK-supermarket search (food-search.ts) + camera barcode scan + "Use a saved meal" (logs every item in a template at once) inside the add-food modal, quantity as grams / a real structured portion chip / a manual custom item; each logged entry also has an "Edit" action next to "Delete" — a modal to change just the quantity, rescaling calories/macros proportionally (updateFoodLogQuantity(), see food-logs.ts)
        saved-meals.tsx    # Saved Meals screen — every saved template with item count/total kcal/item names, a "Log to {slot}" quick action per meal slot (logs to today), delete (confirm-gated); reached from Nutrition's Saved Meals pill, not a tab — and, since an earlier chunk, actually hidden from the tab bar too (see _layout.tsx)
        form-check.tsx     # reached from Training's Form Check pill, not a tab (href: null, see _layout.tsx) — name the exercise + optional note, Record or Choose from Library (expo-image-picker, video mode), submit; lists past submissions with Pending/Reviewed status, expanding a Reviewed one shows the coach's feedback text and optional follow-up video; the standard locked upsell card if the coach has toggled Form Check off
        progress.tsx       # Progress tab shell — Metrics/Measure/Exercise/Photos <PillRow> switcher (Metrics first, and the default tab; the shared pill component itself now lives in pill-row.tsx, reused by Training/Nutrition); Compliance was removed from here, but getComplianceScore() itself still runs — it feeds the coach's Clients list badge (see clients/index.tsx / coach-dashboard.ts)
        exercise-progress/
  ...
  components/
    ...
    log-activity-modal.tsx           # <LogActivityModal visible clientId onClose onSaved> — the client's Log Activity flow, opened from Training's top pill row: activity-type chip grid (+ a "+ Custom" dashed chip revealing a free-text name), duration (required), distance + km/mi toggle/calories/notes (optional); same modal/chip shape nutrition.tsx's food-add modal already established
    pill-row.tsx                      # <PillRow items activeKey?> — the one pill-row look used everywhere a screen needs a row of top-level actions right under its title (Progress's Metrics/Measure/Exercise/Photos switcher, Training's View Calendar/Log Activity/Form Check, Nutrition's Saved Meals); activeKey renders just that one pill Accent-filled/selected (Progress's own use); left unset, every pill in the row renders red/white by default (a plain navigation row has no "selected" concept)
  ...
  lib/
    ...
    activity-logs.ts          # ACTIVITY_TYPES (Run/Swim/Bike/Walk/Row/Pilates/Stairmaster/Activity, each with an Ionicon) / addActivityLog() / listMyActivityLogs() / deleteActivityLog() / activityLabel() (the fixed type's label, or the client's own typed-in name when custom) — a client logging a run/swim/ride/etc that isn't part of a prescribed workout
    plan-upgrades.ts           # PLAN_UPGRADE_OPTIONS (Request New Plan (Base Members) / Accelerator Plan / Precision Plan) / startPlanUpgrade() — inserts the client's own pending plan_upgrade_requests row, then calls the create-checkout-session Edge Function with that row's id and returns the real Stripe Checkout URL it hands back; the client's tier itself only ever changes once stripe-webhook confirms payment
    ...
    weight-logs.ts           # saveWeightLog() (upsert, computes weight_trend; body fat %/muscle % default to "leave unchanged" when omitted, vs. explicitly cleared when passed null) / listWeightLogs() / hasWeightLogForDate() / getLatestWeightLog() (single-row read of the most recent weigh-in, used by Home's Weight ring) database calls
    ...
    momentum.ts                # getMomentumScore() — pure calculation (now also reads activity_logs: folds activity days into activeDaysRate exactly like workouts/food/habits, plus a flat +0.5-capped-at-10 bonus if any activity was logged this week — see activityLoggedThisWeek on the returned breakdown); getCurrentWeekRange() exported so other "this week" features (the Leaderboard's weekly XP ranking) share the exact same Monday, not a second copy of the date math
    ...
    coach-dashboard.ts            # getCoachDashboardStats() (active clients, avg Compliance Score, overdue check-ins, open community reports) / getClientsNeedingAttention() (sub-50% compliance, worst first) / getRecentClientActivity() (small dashboard preview: logged meals + completed workouts + logged activities) / getClientActivityFeed() (the full activity.tsx feed: meals + habits + completed workouts + logged activities, each with that client's live Momentum/Compliance Score attached) / subscribeToClientActivity() (realtime: new food_logs, new habit_logs, new activity_logs, assignments updated to completed)
...
supabase/
  functions/
    ...
    create-checkout-session/
      index.ts                    # Edge Function — a client's chosen tier is looked up from their own already-inserted plan_upgrade_requests row (never trusted straight from the client), mapped to one of three hardcoded Stripe Price IDs, and turned into a real Checkout Session; needs STRIPE_SECRET_KEY set as this function's own secret
    stripe-webhook/
      index.ts                    # Edge Function — the only place a client's tier is actually upgraded as a result of real payment; verifies Stripe's signature (STRIPE_WEBHOOK_SECRET), then on checkout.session.completed upserts client_tiers + marks the request completed, both via the service-role key. Must be deployed with JWT verification OFF (Stripe calls it directly, no Supabase session) — deliberately doesn't handle cancellations/renewals/refunds yet, only a successful checkout
  ...
  activity-logs.sql                                                                                                           # paste in after custom-exercises.sql — new activity_logs table (a client's own logged run/swim/ride/etc, not part of a prescribed workout), client-only select/insert/delete + coach select-all (same shape client-activity-feed.sql already gave habit_logs), added to the supabase_realtime publication
  plan-upgrades.sql                                                                                                           # paste in after activity-logs.sql — new plan_upgrade_requests table (one row per Stripe checkout attempt); a client can only ever select/insert their own pending row, the coach can select every row, and nobody (not even the requester) can update or delete one directly — only the two new Edge Functions, using the service-role key, ever move a request past 'pending'
  coach-notifications.sql                                                                                                     # paste in after plan-upgrades.sql — adds roster_last_viewed_at/checkins_last_viewed_at to profiles (both timestamptz not null default now(), each with its own column-level grant) + adds profiles/form_check_ins to the supabase_realtime publication, powering Home's three new coach-side notification badges
  profile-avatar.sql                                                                                                          # paste in after coach-notifications.sql — a private avatars Storage bucket (select/insert/delete RLS keyed on the <user_id>/ folder prefix, same shape progress-photos.sql/recipes.sql already established) + adds avatar_storage_path (nullable) to profiles with its own column-level grant, powering the client profile-picture upload
```
