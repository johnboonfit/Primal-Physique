...
**What was verified here, and what needs your own device:** this sandbox's network proxy blocks both `world.openfoodfacts.org` and `api.nal.usda.gov` outright, so the actual live API responses couldn't be exercised end-to-end here — which is exactly how the `/api/v2/search` bug above got through the first time. What *was* verified, against realistic canned API responses run through the real production code: the merge order (UK results ranked first), the UK-relevance filter (a product tagged `en:united-kingdom` and a separately-tagged product whose brand name alone marks it as UK-relevant both survive; a genuinely non-UK product mentioning the same search term does not), the quality gate on top of that (a UK-relevant product that doesn't actually match the typed search term is still excluded, and an unrelated search like "chicken breast" shows no UK results at all, same as before this change), and all three portion-unit modes computing the right total grams and macros (a structured USDA portion, a multi-count of one, and a manual custom item).
